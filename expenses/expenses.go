...
func TotalByPeriod(in []Record, p DaysPeriod) float64 {
	periods := Filter(in, ByDaysPeriod(p))

	var sum float64
	for _, record := range periods {
		sum += record.Amount
	}
	return sum
}
...
func CategoryExpenses(in []Record, p DaysPeriod, c string) (float64, error) {
	recordsFilter := Filter(in, ByCategory(c))
	if len(recordsFilter) == 0 {
		return 0, errors.New("unknown category " + c)
	}
	return TotalByPeriod(recordsFilter, p), nil
